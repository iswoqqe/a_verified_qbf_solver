...
"trim_ws Nil = Nil" |
"trim_ws (Cons x xs) =
  (if x = CHR '' '' then trim_ws xs else Cons x xs)"

lemma non_increasing_trim_ws [simp]: "length (trim_ws s) \<le> length s"
  by (induction s) auto

lemma [intro]: "length s \<le> length s' \<Longrightarrow> length (trim_ws s) \<le> length s'"
  by (induction s) auto

lemma [intro]: "length s < length s' \<Longrightarrow> length (trim_ws s) < length s'"
  by (induction s) auto

lemma [intro]: "length s \<le> length (trim_ws s') \<Longrightarrow> length s \<le> length s'"
  apply (induction s')
   apply simp
  using le_trans non_increasing_trim_ws by blast

lemma [intro]: "length s < length (trim_ws s') \<Longrightarrow> length s < length s'"
  apply (induction s')
   apply simp
  using non_increasing_trim_ws order.strict_trans2 by blast
...
lemma non_increasing_match [simp]: "match xs s = Some ((), s') \<Longrightarrow> length s' \<le> length s"
  by (induction xs s rule: match.induct) ((case_tac "x = y",auto)+)
...
lemma non_increasing_num_aux [simp]: "num_aux n s = Some (m, s') \<Longrightarrow> length s' \<le> length s"
  by (induction n s rule: num_aux.induct) ((case_tac "List.member ''0123456789'' x",auto)+)
...
lemma decreasing_pnum_raw [simp]: "pnum_raw s = Some (n, s') \<Longrightarrow> length s' < length s"
  apply (cases s)
   apply simp
  apply (metis impossible_Cons nat_less_le non_increasing_num_aux num_aux.simps(2) option.simps(3)
      pnum_raw.simps(2))
  done
